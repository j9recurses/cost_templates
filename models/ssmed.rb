...
validates   :ssmedprint, :ssmedcampm,   numericality:{only_integer: true, :greater_than_or_equal_to => 0, :less_than_or_equal_to  => 30000000,  :allow_nil => true, :allow_blank => false,  message: " Entry is not valid. Please check your entry"  }


  def self.total_steps
   c = CategoryDescription.where(model_name: "ssmeds").pluck(:field, :label)
...
def self.category_status(category_id, model_stuff)
  model_fields =  Ssmed.column_names
